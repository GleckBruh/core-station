...
public sealed partial class SurgerySystem : SharedSurgerySystem
{
    ...
    {
        if (!TryGetSingletonEntity(args.SurgeryProto, out var surgery)
    ...
    private void OnStepOrganInsertComplete(Entity<SurgeryStepOrganInsertComponent> ent, ref SurgeryStepEvent args)
    {
        ...
        var containerId = BodySystem.GetOrganContainerId(ent.Comp.Slot);
    ...
    private void OnStepOrganExtractComplete(Entity<SurgeryStepOrganExtractComponent> ent, ref SurgeryStepEvent args)
    {
        ...
        if (ent.Comp.Slot != null && _containers.TryGetContainer(args.Part, BodySystem.GetOrganContainerId(ent.Comp.Slot), out var container))
        {
    ...
    {
        if (!TryComp<TransformComponent>(args.Body, out var bodyXform)
            || !TryComp<HumanoidAppearanceComponent>(args.Body, out var humanoid)
            || !TryComp<BodyComponent>(args.Body, out var bodyComp)
            || !TryComp<TransformComponent>(args.Part, out var limbXform)
            || !TryComp<MetaDataComponent>(args.Part, out var limbMeta))
        {
            return;
        }

        _limbSystem.Amputate(
            (args.Body, bodyXform, humanoid, bodyComp),
            (args.Part, limbXform, limbMeta));
    }
