...
public sealed partial class SurgerySystem : SharedSurgerySystem
{
    ...
    private void AddSurgeries(EntityUid part, EntityUid body, Dictionary<NetEntity, List<(EntProtoId, string suffix, bool isCompleted)>> surgeries)
    {
        ...
        {
            if (!TryGetSingletonEntity(surgery, out var surgeryEnt)
                || !TryComp<SurgeryComponent>(surgeryEnt, out var surgeryComp)
                || (surgeryComp.Requirement.Count > 0
                    && !progress.CompletedSurgeries.Any(x => surgeryComp.Requirement.Contains(x))))
            {
                continue;
            }
