...
namespace Content.Client._Other.Starlight.Medical.Surgery;
...
public sealed class SurgeryBui : BoundUserInterface
{
    ...
    private bool TryGetSingletonEntity(EntProtoId protoId, out EntityUid uid)
    {
        if (_singletonEntities.TryGetValue(protoId, out uid) && _entities.EntityExists(uid))
            return true;
        ...
        uid = _entities.SpawnEntity(protoId, MapCoordinates.Nullspace);
    ...
    private void Update(SurgeryBuiState state)
    {
        ...
        foreach (var part in parts)
        {
            ...
            {
                if (!TryGetSingletonEntity(surgeryId, out var surgery) ||
                    !_entities.TryGetComponent<SurgeryComponent>(surgery, out var surgeryComp))
                {
                    continue;
                }
    ...
    private void TryInitWindow()
    {
        ...
        _window.StepsButton.OnPressed += _ =>
        {
            ...
            if (!TryGetSingletonEntity(last, out var previousId) ||
                !_entities.TryGetComponent<SurgeryComponent>(previousId, out var previous))
            {
                return;
            }
    ...
    {
        if (_window == null ||
            !TryGetSingletonEntity(stepId, out var step))
        {
            return;
        }
    ...
    private void OnSurgeryPressed(Entity<SurgeryComponent> surgery, NetEntity netPart, EntProtoId surgeryId)
    {
        ...
        if (surgery.Comp.Requirement is { } requirementIds)
        {
            ...
            {
                if (TryGetSingletonEntity(requirementId, out var requirement)
                    ...
                    && _system.IsSurgeryValid(Body, Part, requirementId, surgeryId, out _, out _, out _))
                {
                    ...
                    label.Button.OnPressed += _ =>
                    {
                        ...
                        if (_entities.TryGetComponent<SurgeryComponent>(requirement, out var requirementComp))
    ...
    private void RefreshUI()
    {
        if (_window == null ||
            ...
            !_entities.TryGetComponent<BodyPartComponent>(_part, out var part))
        {
            return;
        }
    ...
    private void View(ViewType type)
    {
        ...
        if (_entities.TryGetComponent<MetaDataComponent>(_part, out var partMeta) &&
            _entities.TryGetComponent<MetaDataComponent>(_surgery?.Ent, out var surgeryMeta))
        {
