...
public sealed partial class BodySystem
{
    ...
    private void RecursiveBodyUpdate(Entity<BodyPartComponent> ent, EntityUid? bodyUid)
    {
        ...
        {
            if (!_container.TryGetContainer(ent, GetOrganContainerId(slotId), out var container))
                continue;

            foreach (var organUid in container.ContainedEntities)
            {
                if (!TryComp(organUid, out OrganComponent? organComp))
                    continue;

                if (organComp.Body != bodyUid)
                {
                    organComp.Body = bodyUid;
                    Dirty(organUid, organComp);
                }

                if (oldBody is { Valid: true } oldBodyUid && oldBodyUid != bodyUid)
                {
                    var removedEv = new OrganGotRemovedEvent(oldBodyUid);
                    RaiseLocalEvent(organUid, ref removedEv);
                }

                if (bodyUid is { Valid: true } newBodyUid && oldBody != newBodyUid)
                {
                    var insertedEv = new OrganGotInsertedEvent(newBodyUid);
                    RaiseLocalEvent(organUid, ref insertedEv);
                }
            }
        }
        ...
        {
            if (!_container.TryGetContainer(ent, GetPartSlotContainerId(slotId), out var container))
                continue;
        ...
        string slotId)
    {
        ...
        partEnt.Comp.Body = bodyEnt.Owner;

        var ev = new BodyPartAddedEvent(slotId, partEnt);
        RaiseLocalEvent(bodyEnt.Owner, ref ev);
    }
    ...
    /// Does not validate whether this is part of a body graph.
    /// </summary>
        ...
        [NotNullWhen(true)] out BodyPartComponent? parentComponent)
    {
        ...
        if (_container.TryGetContainingContainer((partUid, null, null), out var container) &&
        ...
        BodyPartComponent? part = null)
    {
        ...
        _container.EnsureContainer<ContainerSlot>(partId.Value, GetPartSlotContainerId(slotId));
        ...
        BodyPartComponent? part = null)
    {
        return Resolve(partId, ref part, logMissing: false)
            ...
            && _container.TryGetContainer(parentId, GetPartSlotContainerId(slotId), out var container)
            && _container.CanInsert(partId, container);
    }
        ...
        BodyPartComponent? part = null)
    {
        ...
        if (!_container.TryGetContainer(parentPartId, GetPartSlotContainerId(slot.Id), out var container))
        {
            DebugTools.Assert($"Unable to find body slot {slot.Id} for {ToPrettyString(parentPartId)}");
            return false;
        }

        return _container.Insert(partId, container);
    }
    ...
    public IEnumerable<BaseContainer> GetPartContainers(EntityUid id, BodyPartComponent? part = null)
    {
        ...
        foreach (var slotId in part.Children.Keys)
        {
            ...
            if (!_container.TryGetContainer(id, containerSlotId, out var container))
                continue;
    ...
    public IEnumerable<Entity<BodyPartComponent>> GetAllBodyPart(EntityUid partId, BodyPartComponent? part = null)
    {
        if (!Resolve(partId, ref part, logMissing: false))
            yield break;
    ...
    public bool BodyHasChild(
        ...
        BodyPartComponent? part = null)
    {
        // Your current BodyComponent does not have a Sunrise RootContainer.
        // Compatibility behavior: if partId is an organ in the normal body organ container, accept it;
        // otherwise, check if partId itself is a body part tree root.
        if (Resolve(bodyId, ref body, logMissing: false)
            && body.Organs != null
            && body.Organs.ContainedEntities.Contains(partId))
            return true;
        ...
        BodyComponent? body = null)
    {
        ...
        // New compatibility path: body parts are not stored in your old organ container.
        // Search known BodyPartComponent entities linked to this body.
        var query = EntityQueryEnumerator<BodyPartComponent>();
        while (query.MoveNext(out var uid, out var part))
        {
            if (part.Body == bodyId.Value)
    ...
    public IEnumerable<EntityUid> GetBodyPartAdjacentParts(EntityUid partId, BodyPartComponent? part = null)
    {
        ...
        foreach (var slotId in part.Children.Keys)
        {
            var container = _container.GetContainer(partId, GetPartSlotContainerId(slotId));
