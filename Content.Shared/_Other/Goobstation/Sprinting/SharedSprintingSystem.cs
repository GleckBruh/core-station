...
using Content.Shared._Core.Movement;
...
public abstract class SharedSprintingSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var sprinter, out var stamina))
        {
            ...
            if (curTime < sprinter.NextDrainTick)
                continue;

            sprinter.NextDrainTick = curTime + TimeSpan.FromSeconds(1f);

            _staminaSystem.TakeStaminaDamage(
                uid,
                sprinter.StaminaDrainRate,
                stamina,
                source: uid,
                visual: false);
    ...
    private void OnDisarm(EntityUid uid, SprinterComponent sprinter, ref DisarmedEvent args)
    {
        ...
        _staminaSystem.TakeStaminaDamage(uid, sprinter.StaminaPenaltyOnShove, source: uid);
