...
public abstract class SharedFlightSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<FlightComponent, StaminaComponent>();
        var curTime = _timing.CurTime;

        while (query.MoveNext(out var uid, out var component, out var stamina))
        {
            if (!component.On)
                continue;
            ...
            // Core for not Goob fix
            if (curTime >= component.NextDrainTick)
            {
                component.NextDrainTick = curTime + TimeSpan.FromSeconds(1f);

                _staminaSystem.TakeStaminaDamage(
                    uid,
                    component.StaminaDrainRate * component.StaminaDrainMultiplier,
                    stamina,
                    uid);
    ...
    public void ToggleActive(EntityUid uid, bool active, FlightComponent component, bool gracefulStop = true)
    {
        ...
        if (active)
        {
            component.NextDrainTick = _timing.CurTime + TimeSpan.FromSeconds(1f);
            _audio.PlayPredicted(component.FlapSound, uid, uid);
        }
